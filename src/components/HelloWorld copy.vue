<template>
  <el-form ref="form" :model="form">
    ...
    <el-row>
      <el-col :span="6">
        ...
        <el-select v-model="form.provinceValue1" placeholder="选择" @change="selectProvince1">
          <el-option
      ...
      <el-col :span="6">
        ...
        <el-select v-model="form.cityValue1" placeholder="请选择" @change="selectcity1">
          <el-option
  ...
  <el-form-item label="入职日期" required>
    <el-col :span="15">
      <el-form-item prop="entryDate">
        <el-date-picker type="date" placeholder="选择日期" v-model="ruleForm.entryDate" value-format="yyyy-MM-dd" style="width: 100%;"></el-date-picker>
      </el-form-item>
    </el-col>
  ...
  <el-form-item label="电话" prop="phone">
    <el-input v-model="ruleForm.phone"></el-input>
        ...
        <el-form-item :label-position="right1" label="省">
        <el-select v-model="provinceValue2" placeholder="选择" @change="selectProvince2">
          <el-option
            v-for="(item,index) of provincearr"
            :key="index"
            :label="item.name"
  ...
  <el-form-item :label-position="right1" label="市">
        <el-select v-model="cityValue2" placeholder="请选择" @change="selectcity2">
          <el-option
            v-for="(item,index) of cityarr"
            :key="index"
            :label="item.name"
  ...
  <el-form-item>
    <el-button type="primary" @click="submitForm('ruleForm');insert()">立即创建</el-button>
  ...
  <span slot="footer" class="dialog-footer">
    ...
    >
      ...
      <el-table-column prop="orientation" label="个人定位" width="120"> </el-table-column>
      <el-table-column prop="plan" label="个人规划" width="120"> </el-table-column>
      <el-table-column prop="introduction" label="个人简介" width="120"> </el-table-column>
      <el-table-column prop="provinceValue" label="省" show-overflow-tooltip></el-table-column>
      <el-table-column prop="cityValue" label="市" show-overflow-tooltip></el-table-column>
      <el-table-column prop="regionValue" label="区" show-overflow-tooltip></el-table-column>
       <el-table-column label="操作">
      <template slot-scope="scope">
        <el-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row);dialogVisible2 = true">修改</el-button>
          <el-dialog
  title="提示"
  ...
  <el-form-item label="入职日期" required>
    <el-col :span="15">
      <el-form-item prop="entryDate">
        <el-date-picker type="date" placeholder="选择日期" v-model="ruleForm2.entryDate" value-format="yyyy-MM-dd" style="width: 100%;"></el-date-picker>
      </el-form-item>
    </el-col>
    <el-col class="line" :span="2"></el-col>
  </el-form-item>
  ...
  <el-form-item label="电话" prop="phone">
    <el-input v-model="ruleForm2.phone"></el-input>
        ...
        <el-form-item :label-position="right1" label="省">
        <el-select v-model="provinceValue3" placeholder="选择" @change="selectProvince3">
          <el-option
            v-for="(item,index) of provincearr"
            :key="index"
            :label="item.name"
  ...
  <el-form-item :label-position="right1" label="市">
        <el-select v-model="cityValue3" placeholder="请选择" @change="selectcity3">
          <el-option
            v-for="(item,index) of cityarr"
            :key="index"
            :label="item.name"
  ...
  <span slot="footer" class="dialog-footer">
    ...
    <el-button type="primary" @click="updateEdit(scope.row.id); dialogVisible2 = false">确 定</el-button>
  </span>
</el-dialog>
        <el-button
          size="mini"
          type="danger"
          @click="removeData(scope.row.id)">删除</el-button>
      </template>
    </el-table-column>
    </el-table>
    <el-pagination
    ...
    return {
      ...
      options: [{
        ...
        rules: {
          ...
          orientation: [
            { orientation: 'array', required: true, message: '请至少选择一个个人定位', trigger: 'change' }
          ]
        },
        ruleForm2: {
          name: '',
  ...
  handleCurrentChange (currentPage) {
    ...
    removeData (id) {
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        this.$axios.delete('api/user/deleteByBatch', { params: { ids: id.toString() } }).then(res => {
          console.log(res)
          if (res.status === 200) {
    ...
    removeAllData () {
      this.$confirm('此操作将永久删除这些文件, 是否继续?', '提示', {
        ...
        // 取出选中项的id组成新数组发出
        let param = this.multipleTable.map(x => { return x.id })
        this.$axios.delete('api/user/deleteByBatch', { params: { ids: param.toString() } }).then(res => {
          if (res.status === 200) {
            this.$message.success('删除成功 !')
            this.queryPage()
          } else {
